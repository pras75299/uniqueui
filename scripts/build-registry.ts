...
async function renderDemosConfig(manifest: RegistryDocsManifest) {
  ...
  // Match either the current `registry/components/*.json` banner or the legacy
  // `registry/docs.json` banner, so a demos source carried over from before the
  // ADR 0002 split gets its old header replaced rather than double-stamped.
  const bodyWithoutBanner = demosSource.replace(
    /^\/\/ This file is auto-generated by `pnpm build:registry`\.\n\/\/ Source of truth: `(?:registry\/components\/\*\.json|registry\/docs\.json)`(?: -> [^`]+)?\.\n\/\/ Do not edit this file manually\.\n\n/,
    "",
  );
