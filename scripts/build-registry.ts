...
async function syncVersionConstant() {
  const pkg = (await fs.readJson(CLI_PACKAGE_FILE)) as { version?: string };
  if (typeof pkg.version !== "string" || pkg.version.length === 0) {
    throw new Error(`syncVersionConstant: missing "version" in ${CLI_PACKAGE_FILE}`);
  }
  await fs.ensureDir(APP_CONFIG_DIR);
  await fs.writeFile(
    APP_VERSION_FILE,
