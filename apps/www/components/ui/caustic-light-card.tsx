...
export interface CausticLightCardProps
  ...
  /** Tint color for the caustic highlights. */
  causticColor?: string;
  /** Base intensity for the light shimmer. */
  intensity?: number;
  /** Base animation speed. */
  speed?: number;
  /** Fraction of card height affected by caustics (0-1). */
  coverage?: number;
  /** CSS blend mode used by the caustic canvas. */
...
void main() {
  ...
  float verticalMask = 1.0 - smoothstep(coverageStart, 1.0, uv.y);
  float bottomBoost = mix(1.35, 0.55, uv.y);
  float alpha = shimmer * verticalMask * bottomBoost * uIntensity;
...
function hexToRgb(color: string): [number, number, number] {
  ...
  if (!/^[0-9a-fA-F]{6}$/.test(full)) return [1, 0.968, 0.878];
...
>(function CausticLightCard(
  ...
  useEffect(() => {
    ...
    const tint = hexToRgb(causticColor);
