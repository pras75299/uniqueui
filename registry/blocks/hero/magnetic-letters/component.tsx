...
function ConicBackdrop() {
  ...
  return (
    <motion.div
      ...
      style={{
        background:
          "conic-gradient(from 0deg at 50% 50%, transparent 0deg, rgba(168,85,247,0.20) 60deg, transparent 120deg, rgba(56,189,248,0.18) 200deg, transparent 260deg, rgba(168,85,247,0.16) 320deg, transparent 360deg)",
        filter: "blur(60px)",
      }}
      animate={reduced ? undefined : { rotate: 360 }}
      transition={
        reduced ? undefined : { duration: 60, ease: "linear", repeat: Infinity }
      }
    />
  );
}
...
function Vignette() {
  return (
    <div
      aria-hidden
      className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(ellipse_at_center,_transparent_30%,_rgba(8,8,10,0.85)_85%)]"
    />
  );
}
